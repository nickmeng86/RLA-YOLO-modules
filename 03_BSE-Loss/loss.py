...
class SlideWeight:
    def compute(true, auto_iou):
        ...
        weight = a1 * b1 + a2 * b2 + a3 * b3
        return weight

class BSE_Loss:
    """
    BSE_Loss does not implement a loss function on its own.
    Instead, it wraps any given base loss (commonly BCE)
    and applies adaptive, IoU-aware weighting via:
    
    1) EWMA_IoU: Exponential moving average of IoU to generate a dynamic threshold.
    2) SlideWeight: A three-stage piecewise weighting function based on the threshold.
    
    In other words:
        BSE_Loss = BaseLoss × (SlideWeight + EWMA_IoU)
    
    Example (using BCE as the base loss):
        self.bce = BSE_Loss(nn.BCEWithLogitsLoss(reduction='none'))
    
    Dependency and compatibility:
        BSE_Loss requires an additional `auto_iou` during classification loss computation
        to enable per-sample IoU-based adaptive weighting.
    
        To ensure safe operation, the code handles this dependency as follows:
        - Only BSE_Loss receives `auto_iou`.
        - Standard BCE or other base losses are unaffected.
    """
    def __init__(self, loss_fcn, decay=0.999, tau=2000):
        ...
        self.ewma = EWMA_IoU(decay, tau)
        self.is_train = True

    def __call__(self, pred, true, auto_iou=0.5):
        if self.is_train and auto_iou != -1:
            auto_iou = self.ewma.update(auto_iou)

        weight = SlideWeight.compute(true, auto_iou)
        loss = self.loss_fcn(pred, true) * weight
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask,
        ...
        if self.use_wiseiou:
            ...
            # iou = bbox_inner_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True, ratio=0.65)
...
class v8DetectionLoss:
    ...
    def compute_loss(self, preds, batch):
        ...
        if isinstance(self.bce, (BSE_Loss)):
            """
            To ensure compatibility with BSE_Loss, an additional auto_iou value is passed 
            during the classification loss computation to enable IoU-based adaptive weighting. 
            Standard BCE does not require or accept this parameter.
            """
            if fg_mask.sum():
